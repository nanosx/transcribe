...
This is a background utility for macOS that enables system-wide push-to-talk speech transcription. It listens for a global hotkey combination to record audio, transcribes it locally using Whisper model, and pastes it into the field.

The project uses [whisper.cpp](github.com/ggml-org/whisper.cpp) for the inference, for Metal-optimized performance on Apple Silicon. The model used in this example is ggml-small.en-q8_0.bin, a quantised version of [Whisper model](https://huggingface.co/ggerganov/whisper.cpp/tree/main). 
